...
- 결과를 카테고리별로 묶어 `clippings/YYYY-MM-DD.md`에 마크다운으로 저장합니다. 카테고리마다
  최신 기사 최대 3건은 Claude가 (Anthropic 서버의 `web_fetch` 도구로) 기사 원문을 직접 읽고
  `핵심 내용`과 `면접 답변 포인트` 초안을 자동으로 채워줍니다 (AI 초안이므로 원문을 한 번
  확인하고 다듬는 걸 권장합니다). 나머지 기사와 본문을 가져오지 못한 기사는 빈 칸으로 남아
  직접 정리하면 됩니다.
